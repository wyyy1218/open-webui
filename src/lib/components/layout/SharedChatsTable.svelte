...
<div class="w-full rounded-xl border border-gray-100 dark:border-gray-850 overflow-hidden">
	<div class="grid grid-cols-[auto,1fr,9rem,9rem,4.5rem] gap-3 px-4 py-3 text-xs font-medium border-b border-gray-100 dark:border-gray-850 bg-gray-50/50 dark:bg-gray-900/20">
		...
		<button class="text-left flex items-center gap-1.5 select-none {getSortClass('title')}" on:click={() => onToggleSort('title')}>
			{$i18n.t('Title')}
			{#if orderBy === 'title'}
				{#if direction === 'asc'}
		...
		<button class="text-right flex items-center justify-end gap-1.5 select-none {getSortClass('updated_at')}" on:click={() => onToggleSort('updated_at')}>
			{$i18n.t('Updated at')}
		...
		<button class="text-right flex items-center justify-end gap-1.5 select-none {getSortClass('created_at')}" on:click={() => onToggleSort('created_at')}>
			{$i18n.t('Created at')}
	...
	<div class="max-h-[24rem] overflow-y-auto">
		...
		{:else}
			{#each chatList as chat (chat.id)}
				<div
					class="grid grid-cols-[auto,1fr,9rem,9rem,4.5rem] gap-3 px-4 py-3 text-sm border-b border-gray-50 dark:border-gray-850/70 last:border-b-0 hover:bg-gray-50/70 dark:hover:bg-gray-900/30 {selectedChatIds.has(chat.id)
